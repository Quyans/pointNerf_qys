from re import L
import sys
import os
import pathlib
sys.path.append(os.path.join(pathlib.Path(__file__).parent.absolute(), '..'))

from options import TrainOptions


from run import train_ft_allModel




from asyncio import subprocess
import torch.nn as nn
import sys
import os
import pathlib
import argparse
import open3d as o3d
import torch.cuda
from plyfile import PlyData, PlyElement
import numpy as np
sys.path.append(os.path.join(pathlib.Path(__file__).parent.absolute(), '..'))
from utils_mine.utilize import  *


#np.set_printoptions(suppress=True)  # 取消默认科学计数法，open3d无法读取科学计数法表示

import cv2
from tqdm import tqdm
from Editor.pointcloud import *
from Editor.checkpoints_controller import *
from Editor.pointcloud_editor import *
class Options:
    def __init__(self):
        self.opt = None
        self.parse()
    def parse(self):
        parser = argparse.ArgumentParser(description="Argparse of  point_editor")
        parser.add_argument('--checkpoints_root',
                            type=str,
                            default='/home/vr717/Documents/qys/code/pointnerf/checkpoints/scannet/swapPoint/horse',#/home/slam/devdata/pointnerf/checkpoints/scannet/scene000-T
                            help='root of checkpoints datasets a')
        parser.add_argument('--checkpoints_rootb',
                            type=str,
                            default='/home/vr717/Documents/qys/code/pointnerf/checkpoints/scannet/swapPoint/guanZi',#/home/slam/devdata/pointnerf/checkpoints/scannet/scene000-T
                            help='root of checkpoints datasets b')
        parser.add_argument('--gpu_ids',
                            type=str,
                            default='0',
                            help='gpu ids: e.g. 0  0,1,2, 0,2')

        self.opt = parser.parse_args()

        # print(self.opt.dataset_dir)

def test_load_checkpoints_save_as_ply(opt,savename):
    '''
    测试从checkpoints转ply
    '''
    cpc = CheckpointsController(opt)
    neural_pcd = cpc.load_checkpoints_as_nerualpcd()
    neural_pcd.save_as_ply(savename)
# def test_edit(opt):
#     cpc = CheckpointsController(opt)
#     scene_npcd = Neural_pointcloud(opt)
#     scene_npcd.load_from_ply('guanZi')
#     object_mpcd = Meshlab_pointcloud(opt)
#     object_mpcd.load_from_meshlabfile('sofa1')
#     object_npcd = object_mpcd.meshlabpcd2neuralpcd(scene_npcd)
#     object_npcd.save_as_ply('sofa1')
#     pce = PointCloudEditor(opt)
#     R = cauc_RotationMatrix(0, 0, 15)
#     transMatrix = cauc_transformationMatrix(R, np.array([0, -0.8, 0]))
#     transed_sofa = pce.translation_point_cloud_local(object_npcd,transMatrix)
#     transed_sofa.save_as_ply('sofa1_trans(0,0,15)(0,-0.8,0)')
#     new_scene = pce.add_point_cloud(transed_sofa,scene_npcd)
#     new_scene.save_as_ply('scene_add_sofa1(0,0,15)(0,-0.8,0)')
#     cpc.save_checkpoints_from_neuralpcd(new_scene,'scene_add_sofa1(0,0,15)(0,-0.8,0)')

# def test_edit(opt):
#     # //把horse放罐子里
#     cpc = CheckpointsController(opt)
#     scene_npcd = Neural_pointcloud(opt)
#     scene_npcd.load_from_ply('guanZi')
#     # object_mpcd = Meshlab_pointcloud(opt)
#     # object_mpcd.load_from_meshlabfile('sofa1')
#     # object_npcd = object_mpcd.meshlabpcd2neuralpcd(scene_npcd)
#     object_npcd = Neural_pointcloud(opt)
#     object_npcd.load_from_ply('horse')
#     pce = PointCloudEditor(opt)
#     R = cauc_RotationMatrix(0, 0, 0)
#     transMatrix = cauc_transformationMatrix(R, np.array([0, -1, 2]))
#     transed_sofa = pce.translation_point_cloud_local(object_npcd,transMatrix)
#     transed_sofa.save_as_ply('horse_trans(0,0,0)(0,-1,2)')
#     new_scene = pce.add_point_cloud(transed_sofa,scene_npcd)
#     new_scene.save_as_ply('scene_add_horse(0,0,0)(0,-1,2)')
#     cpc.save_checkpoints_from_neuralpcd(new_scene,'scene_add_horse(0,0,0)(0,-1,2)')

def test_edit(opt):
    # 把guanzi放马里
    cpc = CheckpointsController(opt)
    scene_npcd = Neural_pointcloud(opt)
    scene_npcd.load_from_ply('horse')
    object_mpcd = Meshlab_pointcloud(opt)
    object_mpcd.load_from_meshlabfile('guanZi')
    object_npcd = object_mpcd.meshlabpcd2neuralpcd(object_mpcd)
    object_npcd.save_as_ply('guanZi')
    pce = PointCloudEditor(opt)
    R = cauc_RotationMatrix(0, 0, 15)
    transMatrix = cauc_transformationMatrix(R, np.array([0, -0.8, 0]))
    transed_sofa = pce.translation_point_cloud_local(object_npcd,transMatrix)
    transed_sofa.save_as_ply('sofa1_trans(0,0,15)(0,-0.8,0)')
    new_scene = pce.add_point_cloud(transed_sofa,scene_npcd)
    new_scene.save_as_ply('scene_add_guanZi(0,0,15)(0,-0.8,0)')
    cpc.save_checkpoints_from_neuralpcd(new_scene,'scene_add_guanZi(0,0,15)(0,-0.8,0)')


def test_edit1(opt):# sofa only
    cpc = CheckpointsController(opt)
    object_mpcd = Meshlab_pointcloud(opt)
    scene_npcd = Neural_pointcloud(opt)
    scene_npcd.load_from_ply('scene_origin')
    object_mpcd.load_from_meshlabfile('sofa1')
    object_npcd = object_mpcd.meshlabpcd2neuralpcd(scene_npcd)
    cpc.save_checkpoints_from_neuralpcd(object_npcd,'sofa1')

def test_edit2(opt):# delete
    cpc = CheckpointsController(opt)
    object_mpcd = Meshlab_pointcloud(opt)
    scene_npcd = Neural_pointcloud(opt)
    scene_npcd.load_from_ply('scene_origin')
    object_mpcd.load_from_meshlabfile('sofa1')
    object_npcd = object_mpcd.meshlabpcd2neuralpcd(scene_npcd)
    pce = PointCloudEditor(opt)
    bg_npcd = pce.crop_point_cloud(object_npcd,scene_npcd)
    cpc.save_checkpoints_from_neuralpcd(bg_npcd,'bg_nosofa1')

import os
import sys
import time
import subprocess
# def main():
#     sparse = Options()
#     opt = sparse.opt
#     optb = Options().opt
#     optb.checkpoints_root = opt.checkpoints_rootb
#     print(opt.checkpoints_root)
#     print(optb.checkpoints_root)
    
    
    
#     # test_load_checkpoints_save_as_ply(opt,'scene_origin')
#     # 测试读ply:这一步中间，用mesh手抠一个物体，命名为sofa_meshlabpcd.ply~！~！~！~！~！~！~！~！
#     # test_edit(opt)
#     # test_edit1(opt)
#     # test_edit2(opt)
    
#     # test_load_checkpoints_save_as_ply(opt,"test.ply")
    
#     for i in range(500):
#         cpc = CheckpointsController(opt)
#         cpcb = CheckpointsController(optb)
#         print(opt.checkpoints_root)
#         print(optb.checkpoints_root)
#         print("要训练：",opt.checkpoints_root)
#         cpc.save_checkpoints_from_neuralpcd_cover_by_b(cpcb.latest_file_path)
        
        
#         # 脚本调用
        
#         bash_root = "/home/vr717/Documents/qys/code/pointnerf/dev_scripts/w_scannet_etf/swapBash"
#         model_name = opt.checkpoints_root.split("/")[-1]
#         bash_filename = os.path.join(bash_root,model_name+".sh")
#         print(bash_filename)
        
#         processor = bash_filename
#         pargs = ["bash", processor]
#         # pargs.extend([""+dirPath])
#         p = subprocess.Popen(pargs, shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        
#         # 获取当前时间
#         localtime = time.localtime(time.time())
#         secs = time.mktime(localtime)

        
        
#         while p.poll() is None:
#             localtime1 = time.localtime(time.time())
#             secs1 = time.mktime(localtime1)
#             if (secs1-secs)>35:
#                 p.kill()
#                 break;

#             line = p.stdout.readline()
#             line = line.strip()
#             if line:
#                 print(line)
#                 # print('Subprogram output: [{}]'.format(line))
#             # if p.returncode == 0:
#             #     print('Subprogram success')
#             # else:
#             #     print('Subprogram failed')

#         temp_cp = opt.checkpoints_root;
#         opt.checkpoints_root =  optb.checkpoints_root
#         optb.checkpoints_root = temp_cp

    
   
#     # b模型
#     print(cpc.latest_file_path)
#     print(cpcb.latest_file_path)
   

class MyOpt:
    checkpoints_root = ""
    gpu_ids = ""


def main():
    
    # 在这里定义有几个场景，进行多次训练
    
    train_step = 50
    
    scene0 = {
        "name":"swapPoint/guanZi",
        "scan":"guanZi43/guanZi43short",
        "checkpoints_root":"/home/vr717/Documents/qys/code/pointnerf/checkpoints/scannet/swapPoint/guanZi",
        "train_step":train_step,
        "feat_grad":1,
        "conf_grad":1,
        "dir_grad":1,
        "color_grad":1
    }
    
    scene1 = {
        "name":"swapPoint/horse",
        "scan":"horse43/horse43grid",
        "checkpoints_root":"/home/vr717/Documents/qys/code/pointnerf/checkpoints/scannet/swapPoint/horse",
        "train_step":train_step,
        "feat_grad":1,
        "conf_grad":1,
        "dir_grad":1,
        "color_grad":1
    }
    
    # 所有场景都放在这一个dict中
    scene_list = {
        0:scene0,
        1:scene1
    }
    sceneListSize = 2
    
    
    optList = []
    for i in range(sceneListSize):
        opt_t =  MyOpt()
        opt_t.gpu_ids = "0"
        opt_t.checkpoints_root = scene_list[i]["checkpoints_root"]
        optList.append(opt_t)
    
    # sparse = Options()
    # opt = sparse.opt
    # optb = Options().opt
    # optb.checkpoints_root = opt.checkpoints_rootb
    # print(opt.checkpoints_root)
    # print(optb.checkpoints_root)
    
    # test_load_checkpoints_save_as_ply(opt,"test.ply")
    
    # for i in range(500):
    #     cpc = CheckpointsController(opt)
    #     cpcb = CheckpointsController(optb)
    #     print(opt.checkpoints_root)
    #     print(optb.checkpoints_root)
    #     print("要训练：",opt.checkpoints_root)
    #     cpc.save_checkpoints_from_neuralpcd_cover_by_b(cpcb.latest_file_path)
        
        
    #     # 脚本调用
        
    #     bash_root = "/home/vr717/Documents/qys/code/pointnerf/dev_scripts/w_scannet_etf/swapBash"
    #     model_name = opt.checkpoints_root.split("/")[-1]
    #     bash_filename = os.path.join(bash_root,model_name+".sh")
    #     print(bash_filename)
        
    #     processor = bash_filename
    #     pargs = ["bash", processor]
    #     # pargs.extend([""+dirPath])
    #     p = subprocess.Popen(pargs, shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        
    #     # 获取当前时间
    #     localtime = time.localtime(time.time())
    #     secs = time.mktime(localtime)

        
        
    #     while p.poll() is None:
    #         localtime1 = time.localtime(time.time())
    #         secs1 = time.mktime(localtime1)
    #         if (secs1-secs)>35:
    #             p.kill()
    #             break;

    #         line = p.stdout.readline()
    #         line = line.strip()
    #         if line:
    #             print(line)
    #             # print('Subprogram output: [{}]'.format(line))
    #         # if p.returncode == 0:
    #         #     print('Subprogram success')
    #         # else:
    #         #     print('Subprogram failed')

    #     temp_cp = opt.checkpoints_root;
    #     opt.checkpoints_root =  optb.checkpoints_root
    #     optb.checkpoints_root = temp_cp

    # # b模型
    # print(cpc.latest_file_path)
    # print(cpcb.latest_file_path)
    
    swap_num = TrainOptions().parse_get_opt().swap_num
    if swap_num < sceneListSize:
        #每个场景先训练一个基本的
        print("swapNum:",swap_num)
        extraopt =  scene_list[swap_num]
        train_ft_allModel.main_extra(extraopt)
    else:
        
        baseInd = swap_num%sceneListSize
        nextInd = (swap_num+1)%sceneListSize
        print("OutswapNum:",swap_num)
        
        optbase = optList[baseInd]
        optnext = optList[nextInd]
        cpcbase = CheckpointsController(optbase)
        cpcnext = CheckpointsController(optnext)
        print("base:",optbase.checkpoints_root)
        print("next",optnext.checkpoints_root)
        print("要训练：",optnext.checkpoints_root)
        # 把cpcbase的参数赋值给cpcnext
        
        print("base的latest file path：",cpcbase.latest_file_path)
        cpcnext.save_checkpoints_from_neuralpcd_cover_by_b(cpcbase.latest_file_path)
        # 
        extraopt =  scene_list[nextInd]
        train_ft_allModel.main_extra(extraopt)
        
    
    # for i in range(sceneSize):
    #     train_ft_allModel.main_extra(opt)
    
    # maxSwap = 20000
    # for i in range(maxSwap):
    #     # 交换
    #     baseInd = i%sceneSize
    #     nextInd = (i+1)%sceneSize
    
    # train_ft.main();


if __name__ == '__main__':
    main()
